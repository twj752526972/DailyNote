...
* 麦乐克开机默认进烟感，为了解决产测问题，可以通过在flash的某个offset位置处，读到value值，然后决定是否进MP mode，default是全1，默认进factory mode，MP做完以后，将该value改写为0，则是normal image，写flash的操作是：AT^FLASH=WW,xxxxx,ddd，WW是write words，RW是read words。
    ```sh
    if(check_factory_mode_header_valid() == FALSE)
    {
        printf("Enter Factory Mode\r\n");
        osThreadTerminate(osThreadGetId());
    }

    BOOL check_factory_mode_header_valid(void)
    {
        u32 read_value;
        flash_t flash_obj;
        
        flash_stream_read(&flash_obj, FACTORY_HEADER_OFFSET, sizeof(read_value), (u8*)&read_value);
        FACTORY_DBG_PRINTF("%s, read factory_header 0x%x = 0x%x\r\n", __func__, FACTORY_HEADER_OFFSET, (unsigned int)read_value);

        if((read_value & FACTORY_HEADER_BITS) == FACTORY_HEADER_VALID)
        {
            printf("factory header valid\r\n");
            return TRUE;
        }
        else
        {
            printf("factory header invalid\r\n");
            return FALSE;
        }
    }
    ```
*   对比Genus这边，开机默认进入normal image，全1，也可以仿照上述方式，只要做认证的时候，将某个bit拉为0，但是认证做完以后，写为1，需要erase flash sector，是以4K为单位的，如果开机从nvram去read item，发现并不需要nvram task起来，所以可以改写为
    ```sh
    nvram_id = ((NVM_DATA_TYPE_SYS << NVM_DATA_TYPE_SHIFT) | NVM_SYS_ID_IMEISV);
    if (AMIF_GetModemNvram)
    {
        nvram = AMIF_GetModemNvram(nvram_id);
    }
    else
    {
        nvram = modem_nvram;    //确认是哪块nvram
    }

    if (!nvram)
    {
        printf("[ERROR]nvram is NULL\r\n");
        return AT_ERROR;
    }

    nvram_read_item(nvram, ((nvram_type << NVM_DATA_TYPE_SHIFT) | nvram_id), dsat_exec_mp_nvram_read_cmd_callback,cb_data);
    ```
    从read函数的call back function中，根据读到的value，如果是0，则停掉nic_main_task即可。